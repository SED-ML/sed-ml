...
\subsubsection[\element{MappingKind}]{Type \element{MappingKind}}
\label{type:mappingKind}

The \element{MappingKind} primitive data type is used in the definition of the \FitMapping class.  \element{MappingKind} is derived from type \element{string} and its values are restricted to being one of the following possibilities: \val{parameter} and \val{variable}.  Attributes of type \element{MappingKind} cannot take on any other values.  The meaning of these values is discussed in the context of the \FitMapping class's definition in \ref{class:fitMapping}.


\subsubsection[\element{ExperimentKind}]{Type \element{ExperimentKind}}
\label{type:experimentKind}

The \element{ExperimentKind} primitive data type is used in the definition of the \FitExperiment class.  \element{ExperimentKind} is derived from type \element{string} and its values are restricted to being one of the following possibilities: \val{steadyState} and \val{timeCourse}.  Attributes of type \element{ExperimentKind} cannot take on any other values.  The meaning of these values is discussed in the context of the \FitExperiment class's definition in \ref{class:fitExperiment}.


\subsubsection[\element{ScaleKind}]{Type \element{ScaleKind}}
\label{type:scaleKind}

The \element{ScaleKind} primitive data type is used in the definition of the \Bounds class.  \element{ScaleKind} is derived from type \element{string} and its values are restricted to being one of the following possibilities: \val{lin} \val{log}, and \val{log10}.  Attributes of type \element{ScaleKind} cannot take on any other values.  The meaning of these values is discussed in the context of the \Bounds class's definition in \ref{class:bounds}.

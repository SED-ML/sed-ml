...
The optional \element{modelReference} attribute of type \SIdRef must, if defined, refer to a \Model.  Inside a \RepeatedTask, the state of the model may have been changed, otherwise, the \Model is assumed to be in its initial state.  If the child \Algorithm is not defined, this attribute is required. 

\paragraph*{\element{simulationReference}}
The optional \element{simulationReference} attribute of type \SIdRef must, if defined, refer to a \Simulation.  If the child \Algorithm is not defined, this attribute is required.
...
The \concept{FunctionalRange} (\fig{range}) constructs a range through calculations that determine the next value based on the value(s) of other range(s) or model variables. It inherits an optional child \Algorithm from \Range, adds the optional attribute \code{range}, and also inherits from \Calculation, from which it gets the three optional children \Math, \ListOfVariables, and \ListOfParameters.

...
Each instance of the \concept{Model} class has the required attributes \hyperref[sec:id]{\element{id}}, \hyperref[sec:model_source]{\code{source}}, and \hyperref[sec:language]{\element{language}}, the optional attribute \hyperref[sec:name]{\element{name}}, and the optional child \hyperref[sec:changesModel]{\element{listOfChanges}}.
...
The \changed{optional} \concept{\element{target}} attribute holds an unambiguous description of the address of an element or attribute of a model that is to undergo the defined changes. For XML model languages such as SBML, \hyperref[sec:changeTarget]{\element{target}} must be a valid \hyperref[sec:xpath]{XPath} expression of data type \hyperref[type:xpath]{\element{xpath}} pointing to the XML that is to undergo the defined changes.  \changed{If the \Change has no child \concept{algorithm} child defined, the \concept{target} is required.}
...
\begin{enumerate}
	...
	\item{If it is used as a sub-element of the \hyperref[class:changeXml]{\element{changeXML}} element, then the XML it contains \emph{replaces} the XML element or elements addressed by the XPath.}
\end{enumerate}
...
The XPath expression is specified in the required \hyperref[sec:changeTarget]{\element{target}} attribute, and may target one or more XML elements. The replacement XML content is specified in the \hyperref[sec:newXml]{NewXML} class, and may also contain one or more XML elements.
...
The \concept{RemoveXML} class can be used to delete one or more XML elements or attributes in the model that are addressed by the XPath expression (\fig{sedChange}). The XPath is specified in the required \hyperref[sec:changeTarget]{\element{target}} attribute.
...
Despite its name, the `attribute' changed by this class need not be an XML attribute, and hence, its \element{target} need not be an \XPath.  Every target model language may define what `attributes' may be changed by this contruct, and how to indicate those attributes.
...
\label{sec:newValue}
The mandatory \concept{\element{newValue}} attribute of data type \code{string} assignes a new value to the targeted attribute.

...
The SED-ML \concept{Parameter} class creates named parameters with a constant value (\fig{parameter}).
The \concept{Parameter} class introduces the following attributes: the required attribute \hyperref[sec:id]{\element{id}} and \hyperref[sec:value]{\element{value}}, and the optional attribute \hyperref[sec:name]{\element{name}}.

\sedfig[width=0.3\textwidth]{images/uml/parameter}{The Parameter class}{fig:parameter}

SED-ML allows the use of \concept{Parameters} wherever a mathematical expression is defined to compute a value (e.g.\ in \hyperref[class:computeChange]{ComputeChange}, \hyperref[class:functionalRange]{FunctionalRange} or \hyperref[class:dataGenerator]{DataGenerator}). Hereby, the parameter definitions are local to the particular class defining them. 
...
\concept{Variables} are references to already existing entities, either to existing objects in one of the defined \hyperref[class:model]{models} or to implicitly defined \hyperref[sec:symbol]{symbols} (\fig{variable}). The \concept{Variable} class introduces the following attributes: the required attribute \hyperref[sec:id]{\element{id}}, the optional attribute \hyperref[sec:name]{\element{name}}, and the context dependent attributes \hyperref[sec:target]{\element{target}}, \hyperref[sec:symbol]{\element{symbol}}, \hyperref[sec:taskReference]{\element{taskReference}}, and \hyperref[sec:modelReference]{\element{modelReference}}.
...
\label{sec:listOf}
SED-ML \concept{listOf*} elements serve as containers  for a collection of objects of the same type. For example, the \code{listOfModels} contains all \hyperref[class:model]{Model} objects of a SED-ML document. Lists do not carry any further semantics nor do they add additional attributes to the language. They might, however, be annotated with \hyperref[class:notes]{Notes} and \hyperref[class:annotation]{Annotations} as they are derived from \hyperref[class:sedBase]{SEDBase}. All \concept{listOf*} elements are optional in a SED-ML document (with exception of \hyperref[sec:listOfRanges]{\element{listOfRanges}} and \hyperref[class:subTask]{\element{listOfSubTasks}} in \hyperref[class:repeatedTask]{RepeatedTask} which are mandatory).
...
The \concept{taskReference} is used to refer to a particular \hyperref[class:abstractTask]{AbstractTask} via a \hyperref[type:sidref]{\element{SIdRef}}. The \concept{taskReference} is used in \hyperref[class:subTask]{SubTask} to reference the respective subtask, or in \hyperref[class:variable]{Variable} within a \hyperref[class:dataGenerator]{DataGenerator}.
...
The \concept{dataReference} is used to refer to a particular \hyperref[class:dataGenerator]{DataGenerator}

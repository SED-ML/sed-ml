...
% TODO: Clarify semantics: XPath needs to address something that has a numerical value, and we calculate a new value.
% TODO (#70): what happens if there is a cyclic dependency between computations? If a ComputeChange uses a variable from another model, are we allowed to assume that changes to the referenced model are performed first (hence disallowing cycles)?

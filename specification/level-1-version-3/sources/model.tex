...
% CHANGE SUBCLASSES
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\subsection{\element{Change} subclasses}
\label{class:changeSubClasses}
...
The computation can use the value of \hyperref[class:variable]{variables} from any model defined in the simulation experiment. Those \hyperref[class:variable]{variables} need to be defined, and can then be addressed by their \hyperref[sec:id]{\element{id}}. A \hyperref[class:variable]{variable} used in a \concept{ComputeChange} must carry a \hyperref[sec:modelReference]{\element{modelReference}} attribute but no \hyperref[sec:taskReference]{\element{taskReference}} attribute (\fig{computeChange}).

To carry out the calculation it may be necessary to introduce additional parameters, that are not defined in any of the models used by the experiment. This is done through the \hyperref[class:parameter]{parameter} class, and such parameters are thereafter refered to by their \hyperref[sec:id]{\element{id}}. 

The mathematical expression for the change is specified using the \hyperref[sec:math]{\element{math}} attribute. The computed new value is hereby described by a mathematical expression using \hyperref[sec:mathML]{MathML}.
...
\label{sec:math}
The \concept{\element{math}} element of data type \hyperref[type:mathml]{\element{MathML}} encodes mathematical expressions. If used as an element of the \concept{ComputeChange} class, it computes the change of the element or attribute addressed by the \hyperref[sec:changeTarget]{target} attribute.

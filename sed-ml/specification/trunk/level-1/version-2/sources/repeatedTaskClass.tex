...
\begin{myXmlLst}{A \code{FunctionalRange} element that returns \code{8} if \code{index} is smaller than \code{1}, \code{0.1} if index is between \code{4} and \code{6} and returns \code{8} otherwise.}{lst:functionalRange2}
...
A \element{setValue} element inherits from the SED-ML \hyperref[class:computeChange]{computeChange} class, which allows it to compute arbitrary expressions involving a number of variables, and parameters. The \element{setValue} adds two additional attributes: \code{range}, \code{symbol} and \code{modelReference}. 

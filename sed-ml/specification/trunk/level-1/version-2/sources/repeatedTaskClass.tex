...
% TODO: Fix example - I think a subTask with simulationReference should be illegal?
...
% TODO: clarify concept of master range, and semantics of multiple ranges.
...
\end{myXmlLst}
% TODO: Be explicit about which base is used for log.
...
% TODO: clarify that functionalRange needs a fixed size range as master (c.f. master range todo above).
...
The \element{listOfChanges} element, when present, has to contain one or more \element{setValue} elements. These elements allow to pre-condition the model that will be used for the next execution of the repeated task. % TODO: reword

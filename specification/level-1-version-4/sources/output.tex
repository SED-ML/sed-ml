...
The \element{legend} attribute defines whether a legend should be displayed (\val{true}) or not (\val{false}).  The position and styling of the legend is unspecified.  If the attribute is not defined, it is up to the tool whether to display the legend or not, and does not mean that the attribute has a default value of \val{false}.  The \element{name} of an \AbstractCurve in a \PlotTwo and  the \element{name} of a \Surface in a \PlotThree are used as labels in the legend, if defined. If not the respective \element{id} should be used as label.

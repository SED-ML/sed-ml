...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.3\textwidth]{images/uml/umlClass}
	\caption{UML class}
	\label{fig:umlClass}    
\end{figure}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.4\textwidth]{images/pdf/classRelation}
...
%% ~~~ AGGREGATION ~~~
The \concept{aggregation} (\fig{umlAggregation}, top) indicates that the objects of the participating classes are connected in a way that one class (\code{Whole}) consists of several parts (\code{Part}).
...
%% ~~~ INHERITANCE ~~~
The \concept{generalisation} (\fig{umlGeneralisation}) allows to extend classes (\code{BaseClass}) by additional properties. The derived class (\code{DerivedClass}) inherits all properties of the base class and defines additional ones.
...
Multiplicity can be defined for both sides of a relationship between classes. The default relationship is ``many to many''. The example in \fig{umlMulti} expresses that a class is given by a professor, and a professor might give one to many classes.

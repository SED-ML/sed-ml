...
The new piece of XML code that is to substitute the XML element addressed by the XPath is provided in the required \code{newXML} attribute. 

An example for a change that adds an additional parameter to a model is given in listing \ref{lst:changeXML}.
%
\begin{myXmlLst}{The \code{changeXML} element with its \code{newXML} attribute}{lst:changeXML}
<model [..]>
 <listOfChanges>

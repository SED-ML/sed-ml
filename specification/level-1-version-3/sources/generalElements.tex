...
Primitive data types comprise the set of data types used in SED-ML classes. Most primitive types in SED-ML are taken from the data types defined in XML Schema 1.0, including \code{string}, \code{boolean}, \code{int}, \code{positiveInteger}, \code{double} and \code{XML}. A few other primitive types are defined by SED-ML itself: 
...
An example for a defined \concept{id} is given in Listing~\ref{lst:id}. The defined model has the \code{id} \code{m00001}. If the model is referenced elsewhere in the SED-ML document, it is referred to by that \code{id}.
...
\label{sec:name}
SED-ML classes may have an optional element \element{name} of data type \code{string}. Names do not have identifying character, i.\,e.\ several SED-ML constituents may have the same name. The purpose of the \code{name} attribute is to store a human-readable name of the constituent, e.\,g.\ for display to the user.
...
The \concept{listOfTasks} element contains the defined \hyperref[sec:task]{tasks} for the simulation experiment (\fig{sed-mlMain}).

...
\changed{Each task is defined independently of the others:  if one task is executed after another, the states of the models must be completely reset so there's no cross-contamination of one task to the next.  This means that the tasks are particularly well suited to being executed in parallel, should that be desired.}

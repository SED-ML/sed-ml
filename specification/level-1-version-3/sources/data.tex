...
To indicate that the \hyperref[sec:target]{\element{target}} of the \hyperref[class:variable]{Variable} is an entity defined within the current SED-ML description (and not an xpath expression) the hashtag (\#) with the reference to an \hyperref[type:id]{\element{id}} is used. 

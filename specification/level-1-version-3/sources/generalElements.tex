...
SED-ML uses the \hyperref[class:variable]{Variable} concept to refer to existing entities inside a model. The container for all variables is  \concept{\element{listOfVariables}}. It includes all variables that need to be defined to either describe a change in the model by means of mathematical equations via \hyperref[class:computeChange]{ComputeChange} (\fig{sedChange} or to set up a \hyperref[class:dataGenerator]{DataGenerator} (\fig{dataGenerator}). The \concept{\element{listOfVariables}} is optional and may contain zero to many variables. 

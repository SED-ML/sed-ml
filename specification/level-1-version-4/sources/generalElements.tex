...
A \Variable (\fig{variable}) is a reference to a mathematical value.  The \Variable class inherits the attributes and children of \SEDBase, changing the attribute \element{id} to be required, and adds several more.  \changed{A \Variable references a single mathematical element (which in some cases may be multidimensional) by using its attributes \element{target}, \element{symbol}, \element{target2}, \element{symbol2}, and \element{term}, either together or separately.  It may also need to reference a particular model in a particular context by using the \element{modelReference} and \element{taskReference} attributes.  Finally, it may reduce the dimensionality of the resulting math by using the \element{dimensionTerm} attribute with one or more \AppliedDimension children, as members of its \ListOfAppliedDimensions child.  All of these additional attributes and child objects are optional, but must together be used to fully define a unique mathematical reference.}
...
\begin{itemize}
        ...
        \item In a \DataGenerator, the task must be defined with the \element{taskReference} attribute.  To be explicit, or if the referenced \AbstractTask uses multiple models, the \element{modelReference} attribute may also be defined.
...
If the \Variable is referencing data directly, neither \element{targetReference} nor \element{modelReference} may be defined.
...
\changed{Together, the \element{taskReference} and \element{modelReference} attributes define a model and its context which the other attributes use to pull data from.}
...
An \AppliedDimension object is exclusively used when the \element{dimensionTerm} of the \Variable is defined.  For example, a variable derived from a \Task inside a \RepeatedTask will have the dimensionality of both.  If the \element{dimensionTerm} of the parent \Variable is the `mean' function (\val{KISAO:0000825}), the following options are available:

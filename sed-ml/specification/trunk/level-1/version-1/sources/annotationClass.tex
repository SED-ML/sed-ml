...
Annotations may contain any valid XML content. No additional namespace declaration is needed for use of the annotation element. However, for the type of XML inside annotation, the according namespaces should be declared, if necessary.
For further guidelines on how to use annotations, we would like to encourage the reading of the according section in the SBML specification (\citep{HBH+10}, pp. 14-16).
...
Listing \ref{lst:annotation} shows the use of the \element{annotation} element in a SED-ML file.
%
\begin{myXmlLst}{The annotation element}{lst:annotation}
<sedML>
  [..]
  <model id="model1" metaID="001" name="Circadian Oscillations" language="urn:sedml:language:cellml" 
...
\end{myXmlLst}
%
In the given example a SED-ML \hyperref[class:model]{model} element is annotated by referencing the original publication the model is build upon. The \element{model} contains an \element{annotation} that uses the model-qualifier \element{isDescribedBy} to link to the external resource \element{urn:miriam:pubmed:10415827}. 

...
All the operations listed above operate on \emph{scalar} values.  \changed{Non-scalar values are allowed in the \Math of \DataGenerator elements, where a \Variable may point to multi-dimensional results from a \Task or from an external \DataSource.  In these cases, any MathML functionality is assumed to apply element-by-element, either from dimensional data to a scalar, or from dimensional data to other dimensional data with identical dimensions.  In other words, if \code{M} and \code{N} are matrices, '\code{M+3}' would add three to every element of \code{M}, and '\code{M+N}' would require that \code{M} and \code{N} have the same dimensions, and the resulting matrix \code{R} would be created such that \code{R(x,y) = M(x,y) + N(x,y)}.}  At this point SED-ML does not define an algebra of vector values.

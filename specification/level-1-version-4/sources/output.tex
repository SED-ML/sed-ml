...
\begin{blockChanged}
The \Plot class inherits the \element{id} and \element{name} attributes from \SedBase, may additionally contain a \element{legend} attribute of type Boolean, and may define two optional \Axis children.

The optional \element{xAxis} and \element{yAxis} children, each of type \Axis, define the axes by which the \Curve or \Surface children are to be interpreted.  If either child is omitted, that axis is undefined, and it is up to the tool whether and how to display any necessary axes, and to decide whether that axis should be linear or logarithmic.

The \element{legend} attribute defines whether a legend should be displayed (\val{true}) or not (\val{false}).  If the attribute is missing, it is up to the tool whether to display the legend or not.  It does not mean that the attribute has a default value of \val{false}.  The position and styling of the legend is unspecified.
\subsection{\element{Plot2D}}
\label{class:plot2D}
...
% ~~~ Axis ~~~
\subsection{\element{Axis}}
\label{class:axis}
The \Axis class is used to define whether an axis for a given \Plot is linear or logarithmic, and how to display it.  It inherits the optional attributes \element{id} and \element{name} from \SedBase (the latter of which can be used as a label), and adds the required attribute \element{type} of type \AxisKind (either 'linear' or 'log10'), as well as the optional attributes \element{min} and \element{max}, both of type \element{double}, \element{grid} of type \element{Boolean}, and \element{style} of type \element{SIdRef}.

\begin{blockChanged}
The \element{type} value of \val{linear} means the axis should be scaled linearly, while a value of \val{log10} indicates it should have a log10 scale.  Other scalings are not possible in this version of SED-ML.
\end{blockChanged}
...
The \element{style} attribute, if present, must be the \element{SId} of a \Style in the same \SedDocument, and can be used to indicate how to display the axis itself, for features such as color and/or line thickness for that axis and its labels.  If not present, any style may be used.  Note that it is possible to suppress an axis from being displayed entirely if the corresponding \Style of an \Axis has a \element{line} with a \element{style} of \val{none}.
...
The \element{dataReference} attribute is of type \element{SIdRef}, and must be the ID of a \DataGenerator element in the same \SedDocument.  The data produced by that particular \DataGenerator fills the according \hyperref[class:dataSet]{dataSet} in the \hyperref[class:report]{report}.
...
\sedfig[width=0.57\textwidth]{images/uml/abstractCurve}{The definition of the SED-ML \AbstractCurve, \Curve, and \ShadedArea classes.}{fig:abstractCurve}
\end{blockChanged}


\begin{blockChanged}
\subsection{\element{AbstractCurve}}

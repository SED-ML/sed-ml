...
SED-ML considers \concept{variables} references to already existing entities, either existing in one of the defined \hyperref[class:model]{models} or externally defined \hyperref[sec:symbol]{symbols} (\fig{variable}). 
%
...
If the variable is defined through a reference to an external entity, then the  \hyperref[sec:symbol]{symbol} attribute is used. It holds a SED-ML \hyperref[sec:uriScheme]{URI}. 
...
\code{Symbol} and \code{target} must not be used together in a single instance of Variable.
...
\end{myXmlLst}
%
Listing \ref{lst:variable} defines a variable \code{v1} (l. 7) to compute a change on a model constituent (referenced by the \code{target} attribute in line 5). The value of \code{v1} corresponds with the value of the targeted model constituent references by the \code{target} attribute in line 8. 
The second variable, \code{v2} (l. 21), is used inside a \code{dataGenerator}. As the variable is \concept{time}, the \code{symbol} attribute is used to refer to the SED-ML URI for time (l. 21).

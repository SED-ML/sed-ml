...
% TODO: make sure all examples are consistent in refering to ranges etc.
...
It consists of an optional attribute \code{range}, two optional elements \hyperref[sec:listOfVariables]{listOfVariables} and \hyperref[sec:listOfParameters]{listOfParameters}, and a required element \element{function}.
...
In the \element{listOfVariables}, \hyperref[class:variable]{variable} elements define identifiers refering to model variables or range values, which may then be used within the \element{function} expression.
...
The order in which to run multiple \code{subTask}s must be specified using \code{order} attributes on the \code{subTask} elements; if these are omitted the ordering is implementation defined.

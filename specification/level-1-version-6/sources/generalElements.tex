\section{General data types and classes}
...
\begin{blockChanged}
\subsubsection[\element{listOfDoubles}]{Type \element{listOfDoubles}}
\label{type:listOfDoubles}

The \element{listOfDoubles} primitive data type is used in the definition of the \SpecificTimeCourse class.  \element{listOfDoubles} is derived from type \element{string} and must be a comma-separated list of \element{double} values.  A comma is legal but not necessary at the end of the list: \code{"3.2"}, \code{"3.2,"}, \code{"3.2, 10.7"}, and \code{"3.2, 10.7, 12.1, 13.0,"} are all examples of legal \element{listOfDoubles}.
\end{blockChanged}
...
An \Algorithm has a mandatory attribute \element{kisaoID} which contains a \hyperref[sec:kisao]{KiSAO} reference to the particular algorithm used by its parent \Simulation, \AbstractTask, \Change, \Output, \Range, or \FitExperiment. In addition, the \Algorithm has an optional \hyperref[class:listOfAlgorithmParameters]{\element{listOfAlgorithmParameters}} child containing \hyperref[class:algorithmParameter]{algorithmParameter} children, which are used to parameterize the \concept{algorithm}.  If the algorithm in question is a hybrid algorithm, the root \Algorithm object's \element{kisaoID} will reference the hybrid algorithm itself, and may have \AlgorithmParameter children indicating which specific algorithms are being combined.
...
The \concept{AlgorithmParameter} class allows to parameterize a particular simulation \hyperref[class:algorithm]{algorithm}. The set of possible parameters for a particular instance is determined by the algorithm that is referenced by the \element{kisaoID} of the enclosing \hyperref[class:algorithm]{algorithm} element (\fig{simulation}). Parameters of simulation algorithms are unambiguously referenced by the mandatory \element{kisaoID} attribute. Their value is set in the mandatory \hyperref[sec:algorithmParameterValue]{\element{value}} attribute.  An \AlgorithmParameter may also have child \AlgorithmParameter elements through a \ListOfAlgorithmParameters.
...
\label{sec:math}
A \Calculation's optional child element \concept{\element{math}} contains a MathML expression used to calculate a value in the context of the \Calculation.  The available subset of mathematical functions and elements which can be used in the \Math element are listed in Section~\hyperref[sec:mathML]{MathML}.  If there is no child \concept{algorithm} of the derived class, or if the child \concept{algorithm} is not defined, then this math child is required.
...
% VARIABLE
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\subsubsection{\element{Variable}}
...
% APPLIEDDIMENSION
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\subsubsection{\element{AppliedDimension}}

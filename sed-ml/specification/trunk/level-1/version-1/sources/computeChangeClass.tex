...
The \concept{ComputeChange} class allows to make changes on any element of the XML file addressable by an XPath expression. The changes  are described by mathematical expressions using a \hyperref[sec:mathML]{subset of MathML}. 
...
The \hyperref[sec:target]{target} attribute contains the XPath addressing the piece of XML that is to be changed. 
It is possible to introduce additional parameters for the mathematics. Therefore, the parameters first need to be defined in the 
\hyperref[sec:listOfParameters]{listOfParameters}. They are then referenced through their ID.
To use model variables for the definition of a mathematical expression, those variables need to be defined in the \hyperref[sec:listOfVariables]{listOfVariables} first, and can then be addressed by their ID.

\paragraph{The \code{math} attribute}
\label{sec:math}
...
In addition, three further parameter are defined: \code{w0=1}, \code{n=2}, and \code{K=1e-6}.

...
Most primitive types in SED-ML are taken from the data types defined in XML Schema 1.0. A few other primitive types are defined by SED-ML itself. What follows are the definitions of the SED-ML-specific types.

\subsubsection{Type ID}
\label{type:id}
The XML Schema 1.0 type ID is identical to the XML 1.0 type ID. The literal representation of this type consists of strings of characters restricted as summarized in Figure~\vref{fig:id}.
...
For a detailed description see also the SBML specification on the ``Type ID'' \citep{HBH+10}.
...
For a detailed description see also the SBML specification on the ``Type SId'' \citep{HBH+10}.
...
% \sedfig[width=0.85\textwidth]{pdf/listOfDataDescriptions}{The SED-ML listOfDataDescriptions container}{fig:listOfDataDescriptions}

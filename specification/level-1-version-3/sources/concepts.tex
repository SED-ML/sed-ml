...
\label{sec:symbol}
Some variables used in a simulation experiment are not explicitly defined in the model, but may be implicitly contained in it. For example, to plot a variable's behaviour over time, that variable is defined in an SBML model, whereas \emph{time} is not explicitly defined. 

SED-ML can refer to such implicit variables via the concept \concept{Symbol}. Such implicit variables are defined using the SED-ML URN scheme \element{urn:sedml:symbol:}\emph{implicit variable}. For example, to refer in a SED-ML file to the definition of \emph{time}, the URN \element{urn:sedml:symbol:time} is used.
...
\bottomrule
\end{tabular}
\caption{Predefined symbols in SED-ML}
\label{tab:symbols}
\end{table}

The latest list of predefined symbols is available from \url{http://sed-ml.org/}.

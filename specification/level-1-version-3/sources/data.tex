...
The \concept{dimensionDescription} contains a \hyperref[class:dimensionDescription]{DimensionDescription} object providing the dimension description of the source data file of the \hyperref{class:dataDescription}[DataDescription].
...
The \concept{DataSource} class introduces three attributes: the required attribute \hyperref[sec:id]{\element{id}} and the optional attributes \hyperref[sec:name]{\element{name}} and \hyperref[sec:indexSet]{\element{indexSet}}. Additionally the \hyperref[sec:listOfSlices]{\code{listOfSlices}} element is defined (\fig{dataDescription}). 
...
Since data elements in NuML can be either values or indices, the \SedDataSource element provides two ways of addressing those elements. The \token{indexSet} attribute allows to address all indices provided by NuML elements with \token{indexType}. 


%% ~~~ INDEX SET ~~~
\subsubsection{\element{indexSet}}
\label{sec:indexSet}
Since data elements in NuML can be either values or indices, the \SedDataSource element provides two ways of addressing those elements. The \token{indexSet} attribute allows to address all indices provided by NuML elements with \token{indexType}. 

For example for the \element{indexSet} \token{time} below, a \hyperref[class:dataSource]{dataSource} would extract the set of all timepoints stored in the index.
...
If the \token{indexSet} attribute is specified the corresponding \token{dataSource} may not define any \token{slice} elements.
...
The \concept{listOfSlices} container holds the \hyperref[class:slice]{Slice} definitions of a \hyperref[class:dataSource]{DataSource} (\fig{dataDescription}). The \concept{listOfSlices} is optional and may contain zero to many \hyperref[class:slice]{Slices}.

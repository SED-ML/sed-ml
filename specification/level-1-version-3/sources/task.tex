...
An \concept{abstractTask} in SED-ML represents the base class for all SED-ML tasks, i.e. \hyperref[class:task]{Task} and \hyperref[class:repeatedTask]{RepeatedTask}. 
...
A task in SED-ML links a \hyperref[class:model]{model} to a certain \hyperref[class:simulation]{simulation} description via their respective identifiers (\fig{abstractTask}), using the \hyperref[sec:modelReference]{modelReference} and the \hyperref[sec:simulationReference]{simulationReference}. The task class receives the \code{id} and \code{name} attributes from the \hyperref[class:abstractTask]{AbstractTask}.
...
The \concept{repeatedTask} class (\fig{repeatedTask}) provides a generic looping construct, allowing complex tasks to be represented by composing separate steps. It performs a specified task (or sequence of tasks) multiple times (where the exact number is specified through a \hyperref[sec:listOfRanges]{range} construct), while allowing specific quantities in the model to be altered at each iteration (as defined in the \hyperref[sec:changes]{listOfChanges}).

\sedfig[width=.8\textwidth]{images/uml/repeatedTask}{The SED-ML RepeatedTask class}{fig:repeatedTask}

The \concept{RepeatedTask} inherits from \hyperref[class:abstractTask]{AbstractTask}. Additionally it has two required attributes \hyperref[sec:rangeAttribute]{range} and \hyperref[sec:resetModel]{resetModel} as well as child elements \hyperref[sec:listOfRanges]{\element{sec:listOfRanges}}, \hyperref[sec:changes]{\element{listOfChanges}} and \hyperref[class:subTask]{\element{listOfSubTasks}}. Of these only \hyperref[sec:changes]{\element{listOfChanges}} is optional.
...
The \element{repeatedTask} class has a required attribute \element{range} of type \code{SIdRef}.
It specifies which \hyperref[sec:listOfRanges]{range} defined in the \element{listOfRanges} this repeated task iterates over.
...
% ~~~ RESET MODEL ~~~
\subsubsection{\element{resetModel}}
...
\subsubsection{\element{listOfSubTasks}}
\label{class:subTask}
...
% ~~~ LIST OF RANGES ~~~
\subsubsection{\element{listOfRanges}}
...
\subsubsection{\element{UniformRange}}
...
\subsubsection{\element{VectorRange}}
...
\subsubsection{\element{FunctionalRange}}

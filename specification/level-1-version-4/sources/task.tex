...
\changed{When the \element{resetModel} attribute is set to \val{true}, the individual repeats of the task may be paralellizable, assuming any child \Range of the \RepeatedTask does not depend on the results of any individual repeat (as is theoretically possible for the \FunctionalRange, but not for the other \Range types.)}
...
\begin{blockChanged}
In some cases, it may be possible to run some subtasks in parallel.  Interpreters may use the \element{order} attribute as a hint in making the decision about what to parallelize, but in general, should be prepared to examine which \Model each \SubTask is modifying and which model or models are being used as input.
\end{blockChanged}

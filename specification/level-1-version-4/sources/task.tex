...
\begin{itemize}
\item The \element{order} attribute of both subtasks must be the same or unset.
\item The subtasks must not change the model state of the same model.
\item If there are subtasks to run with a higher \element{order} value, or if the \element{resetModel} attribute of the \RepeatedTask is \val{false}, it must be possible to obtain and apply the changed model states from all parallel subtasks before running the next subtask.
\end{itemize}
...
\changed{Leaving the \element{order} undefined for a \SubTask implies that the \SubTask may be executed before, after, or in parallel with any other \SubTask.  Giving the same \element{order} to multiple \SubTask elements is an explicit statement that each \SubTask in the group may be executed before, after, or in parallel with any other \SubTask in the group. It is recommended that users always explicitly set the \element{order} attribute for this reason.}
